// Parse down the items and the image array as properties
    ...
    function pushCellMarkupToArr(cellCollection, rankNum, rowLabel) {
        if (rankNum > 0) {
            ...
            cellCollection.push(<div id={`rank-${rankNum}`} className="rank-cell" onDrop={drop} onDragOver={allowDrop} >
                {(item != null) ? <img id={`item-${item.id}`} onDragStart={drag} src={imgArr.find(o => o.id === item.imageId)?.image } draggable="true" /> : null }
            </div>);
